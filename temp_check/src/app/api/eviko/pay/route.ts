...
export async function POST(request: NextRequest) {
    try {
        ...
        const nextOrderNumber = (lastTx?.dailyOrderNumber || 0) + 1;
        ...
        if (customerId) {
            await prisma.customer.update({
                ...
                data: {
                    ...
                    lastVisit: new Date(),
                    bonusPoints: { increment: Math.floor(grandTotal * 0.01) },
                },
            });
        }
                        ...
                        for (const recipe of parsedRecipes) {
                            ...
                            try {
                                ...
                                const ing = await prisma.ubtIngredient.findFirst({
                                    where: { id: recipe.xomashyoId, tenantId }
                                });
                                if (ing) {
