...
export async function PATCH(request: NextRequest) {
    try {
        ...
        // Find order whose ID ends with idSuffix using Prisma
        const allOrders = await prisma.kDSOrder.findMany({
            where: { tenantId, status: { not: "served" } },
            select: { id: true }
        });
        const matchedOrder = allOrders.find(o => o.id.endsWith(idSuffix));
        if (!matchedOrder) return NextResponse.json({ error: "Buyurtma topilmadi" }, { status: 404 });
