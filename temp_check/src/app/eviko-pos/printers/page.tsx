...
export default function PosPrintersPage() {
    ...
    const loadPrinters = useCallback(async () => {
        try {
            const r = await fetch("/api/eviko/printer-heartbeat", { cache: "no-store" });
    ...
    const loadJobs = useCallback(async () => {
        try {
            const r = await fetch("/api/eviko/print-queue?status=pending,claimed,printing,failed&limit=30", { cache: "no-store" });
            const d = await r.json();
            setJobs(d.jobs || []);
        } catch {}
    }, []);
    ...
    const processTick = useCallback(async () => {
        try {
            await fetch("/api/eviko/print-queue/process", { method: "POST" });
        } catch {}
    }, []);
    ...
    const handleDiscover = async () => {
        ...
        try {
            ...
            const r = await fetch("/api/eviko/discover-printers", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
            const d = await r.json();
    ...
    const addPrinter = async (ip: string, port: number, name?: string) => {
        setSavingIp(ip);
        try {
            const r = await fetch("/api/eviko/printers", {
    ...
    const testPrint = async (p: PrinterRow) => {
        setTestingId(p.id);
        try {
            const r = await fetch("/api/eviko/print-queue", {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-800">
            ...
            <header className="sticky top-0 z-20 bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3">
                <button onClick={() => router.push("/eviko")}
