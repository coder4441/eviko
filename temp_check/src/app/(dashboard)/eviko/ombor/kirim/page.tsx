...
export default function OmborKirimPage() {
    ...
    const loadProducts = useCallback(async () => {
        try {
            ...
            const [xomRes, menuRes] = await Promise.all([
                fetch("/api/eviko/xomashyo"),
                fetch("/api/eviko/menu?all=1"),
            ]);
    ...
    const fetchSuppliers = useCallback(async () => {
        try {
            const res = await fetch("/api/eviko/kontragent");
            if (res.ok) {
    ...
    const fetchUsdRate = useCallback(async () => {
        try {
            const res = await fetch("/api/eviko/moliya");
            if (res.ok) {
                const data = await res.json();
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            for (const docId of Array.from(selectedIds)) {
                await fetch("/api/eviko/ombor/kirim", {
                    method: "DELETE",
    ...
    const fetchKirimlar = async () => {
        setIsLoading(true);
        try {
            const res = await fetch("/api/eviko/ombor/kirim");
