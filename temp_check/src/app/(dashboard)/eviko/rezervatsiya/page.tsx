...
export default function RezervatsiyaPage() {
    ...
    const { smartTables, addSmartReservation } = useStore();
    const freeTables = smartTables.filter(t => t.status === "free" || t.status === "reserved");
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const res = await fetch("/api/eviko/tables");
    ...
    // Get reservations from smartTables (reserved status)
    const tableReservations = smartTables
    ...
    const handleSave = async (e: any) => {
        ...
        setIsSaving(true);
        try {
            addSmartReservation(form.tableId, form.customerName, form.reservationTime);
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-4 text-white shadow-xl relative overflow-hidden">
                    ...
                    <p className="text-white font-black text-2xl">{smartTables.filter(t => t.status === "free").length}</p>
                </div>
                <div className="bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl p-4 text-white shadow-xl relative overflow-hidden">
                    ...
                    <p className="text-white font-black text-2xl">{smartTables.filter(t => t.status === "occupied").length}</p>
                </div>
            </div>
            ...
            <div className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-2xl p-5">
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    {smartTables.length === 0 && <p className="text-slate-500 text-sm">Stollar topilmadi. Dashboard → Stollar bo'limidan qo'shing.</p>}
                </div>
            </div>
