...
const POS_BACKGROUNDS = [
    ...
    { id: "c4", value: "#ffffff", name: "Oq", type: "color" },
    { id: "c5", value: "#0ea5e9", name: "Havorang", type: "color" },
    { id: "c6", value: "#10b981", name: "Yashil", type: "color" },
    { id: "c7", value: "#ef4444", name: "Qizil", type: "color" },
    { id: "c8", value: "#8b5cf6", name: "Binafsha", type: "color" },
...
const th = {
    ...
    card:      (d: boolean) => d ? "bg-slate-900/60 backdrop-blur-md border border-white/5 shadow-xl shadow-black/20 text-white" : "bg-white/80 backdrop-blur-md border border-slate-200 shadow-sm text-slate-800",
    header:    (d: boolean) => d ? "bg-slate-950/90 backdrop-blur-md border-white/5" : "bg-white/90 backdrop-blur-md border-slate-200 z-30",
    input:     (d: boolean) => d ? "bg-slate-800/80 text-white placeholder-slate-500 border border-white/5 focus:border-sky-500 focus:ring-1 focus:ring-sky-500" : "bg-white/80 text-[#0078d7] placeholder-slate-400 border border-transparent focus:border-white focus:ring-2 focus:ring-white/50",
    sub:       (d: boolean) => d ? "text-slate-400" : "text-slate-500",
    tabInact:  (d: boolean) => d ? "bg-slate-800/80 backdrop-blur-sm text-slate-100 hover:bg-slate-700 font-bold border border-slate-700" : "bg-slate-100/80 backdrop-blur-sm text-slate-900 hover:bg-slate-200 font-bold shadow-sm",
    panel:     (d: boolean) => d ? "bg-slate-900/60 backdrop-blur-xl text-slate-100" : "bg-sky-50/70 backdrop-blur-xl text-slate-800",
    row:       (d: boolean) => d ? "bg-white/5" : "bg-slate-50/60",
...
// Bridge yo'q yoki lokal server hali tayyor emas bo'lsa — odatdagi /api/eviko/print ga fallback.
async function sendPrintJob(
    ...
    try {
        const res = await fetch("/api/eviko/print", { method: "POST", headers, body: JSON.stringify(payload) });
...
const printKitchenReceipt = async (items: {item:any; qty:number}[], tableName: string, authToken?: string) => {
    try {
        ...
        if (Object.keys(printerGroups).length > 0) {
            ...
            const res = await fetch("/api/eviko/printers").catch(() => null);
            if (res && res.ok) {
...
function MenuPanel({ onConfirm, onPay, kassirPrinterIp, autoPrintReceipt, instantAdd, servicePct = 0, tableName = "Buyurtma" }: {
    ...
    useEffect(() => {
        ...
        fetch("/api/eviko/menu", { headers }).then(r => r.json()).then(d => {
        ...
        if (!kassirPrinterIp) {
            fetch("/api/eviko/printers").then(r => r.ok ? r.json() : null).then(data => {
...
export default function UbtPosPage() {
    ...
    const fetchReservations = useCallback(async () => {
        ...
        if (!token) return;
        try {
            const res = await fetch("/api/eviko/reservations", { headers: { "Authorization": `Bearer ${token}` } });
            if (res.ok) {
    ...
    const handleUpdateTableItem = async (c: any, delta: number) => {
        ...
        const updateLocalAndDb = async () => {
            ...
            await fetch("/api/eviko/orders-db", {
    ...
    const saveTwOrders = async (cartItems: CartItem[], custName: string, custPhone: string, payMethod: string, customerId?: string) => {
        ...
        try {
            await fetch("/api/eviko/takeaway", {
                method: "POST", headers: hdrs,
                body: JSON.stringify({
    ...
    const saveDlOrders = async (cartItems: CartItem[], custName: string, custPhone: string, custAddr: string, payMethod: string, customerId?: string) => {
        ...
        const total = Math.round(cartItems.reduce((s, c) => s + c.item.price * c.qty, 0));
        try {
            await fetch("/api/eviko/yetkazish", {
                method: "POST", headers: hdrs,
                body: JSON.stringify({
    ...
    const handleReservation = async (tableId: string, guestName: string, since: string, customerPhone: string, advance: string, notes: string) => {
        ...
        try {
            await fetch("/api/eviko/reservations", {
                method: "POST", headers: hdrs,
                body: JSON.stringify({ 
                    tableId, 
                    guestName,
    ...
    const handleTransferShot = async (targetTable: SmartTable) => {
        ...
        try {
            ...
            const freshRes = await fetch(`/api/eviko/orders-db?tableId=${selTable.id}`, { headers: getHdrs });
            ...
            const delRes = await fetch(`/api/eviko/orders-db?tableId=${selTable.id}`, { method: "DELETE", headers: hdrs });
            ...
            if (remainingItems.length > 0) {
                const putRes = await fetch("/api/eviko/orders-db", {
                    method: "PUT", headers: hdrs,
            ...
            const targetRes = await fetch(`/api/eviko/orders-db?tableId=${targetTable.id}`, { headers: getHdrs });
            ...
            const postRes = await fetch("/api/eviko/orders-db", {
                method: "POST", headers: hdrs,
                body: JSON.stringify({
            ...
            const [oldRefresh, newRefresh] = await Promise.all([
                fetch(`/api/eviko/orders-db?tableId=${selTable.id}`, { headers: getHdrs }),
                fetch(`/api/eviko/orders-db?tableId=${targetTable.id}`, { headers: getHdrs }),
            ]);
    ...
    const fetchTableOrdersFromDB = useCallback(async (tableId: string) => {
        ...
        if (token) hdrs["Authorization"] = `Bearer ${token}`;
        try {
            const res = await fetch(`/api/eviko/orders-db?tableId=${tableId}&_t=${Date.now()}`, { 
                headers: hdrs,
                cache: "no-store" 
            });
            if (res.ok) {
                ...
                // BUGFIX: DB bo'sh qaytarsa (race condition — POST hali saqlanmagan),
                // local stateni o'chirmaylik — foydalanuvchi taomlarini saqlab qolamiz
                setTableOrders(prev => {
                    const localItems = prev[tableId] || [];
                    // DB dan ma'lumot kelgan bo'lsa — ishonchli, uni ko'rsatamiz
                    // DB bo'sh qaytgan bo'lsa lekin local da bor bo'lsa — localdan foydalanamiz
                    if (items.length === 0 && localItems.length > 0) return prev;
    ...
    const fetchPrinterStatus = () => {
        fetch("/api/eviko/printers/ping").then(r => r.json()).then(data => {
    ...
    const resolveCashierPrinter = async (): Promise<{ ip: string; port: number } | null> => {
        ...
        if (!list || list.length === 0) {
            try {
                const res = await fetch("/api/eviko/printers", { headers: token ? { Authorization: `Bearer ${token}` } : {} });
    ...
    const handlePrintOtchot = async () => {
        ...
        try {
            const token = store.kassirSession?.token || store.deviceSession?.token;
            const res = await fetch("/api/eviko/print", {
                method: "POST",
                headers: {
    ...
    const fetchTwAndDlOrders = useCallback(async () => {
        ...
        try {
            ...
            const resDl = await fetch("/api/eviko/yetkazish", { headers: hdrs });
            if (resDl.ok) {
    ...
    const loadAvailablePrinters = async () => {
        try {
            const res = await fetch("/api/eviko/printers");
            const data = await res.json();
    ...
    const handleLanDiscover = async () => {
        ...
        try {
            ...
            if (ipc?.discoverPrinters) {
                ...
                const res = await fetch("/api/eviko/discover-printers", { method: "POST" });
                const data = await res.json();
    ...
    const handleLanSavePrinter = async (ip: string, port: number, name?: string) => {
        ...
        try {
            ...
            await fetch("/api/eviko/printers", {
                method: "POST",
                headers: hdrs,
    ...
    const handleTablePayDirect = async (method: string, customerId?: string) => {
        ...
        if (remainingOrders.length === 0) {
            fetch(`/api/eviko/orders-db?tableId=${selTable.id}`, { method: "DELETE", headers: hdrs }).catch(() => {});
        } else {
            fetch("/api/eviko/orders-db", {
                method: "PUT", headers: hdrs,
    ...
    const handlePrintReceipt = async (overrideIp?: string) => {
        ...
        if (allOrders.length === 0) {
            try {
                ...
                const hdrs: Record<string, string> = {};
                if (token) hdrs["Authorization"] = `Bearer ${token}`;
                const res = await fetch(`/api/eviko/orders-db?tableId=${selTable.id}`, { headers: hdrs });
                if (res.ok) {
                    const data = await res.json();
        ...
        // 4. Saqlangan printerlar ro'yxati
        const token = store.kassirSession?.token || store.deviceSession?.token;
        let printers: any[] = availablePrinters;
        ...
        // 5. Kassa printerini tanlash: session IP mosi → default kassa → kassa → birinchi
        const sessionIp = overrideIp
            || store.kassirSession?.printerIp
            || store.deviceSession?.printerIp
            || "";
        const target =
            (sessionIp ? printers.find((p: any) => p.ipAddress === sessionIp) : undefined)
            || printers.find((p: any) => p.isDefault && (p.role || "cashier") === "cashier")
            || printers.find((p: any) => (p.role || "cashier") === "cashier")
            || printers[0];

        // 6. Printer umuman yo'q → modal ko'rsatish
        if (!target && !sessionIp) {
        ...
        const hdrsPrint: Record<string, string> = {
            "Content-Type": "application/json",
            ...(token ? { Authorization: `Bearer ${token}` } : {})
        };
                                ...
                                <div className={`rounded-xl shadow-2xl border overflow-hidden ${dark ? "bg-gray-800 border-gray-700 shadow-black/50" : "bg-white border-gray-200 shadow-gray-200/50"}`}>
                                    <div className="p-3">
                                        ...
                                        <div className={`pt-3 border-t ${dark ? "border-gray-700" : "border-gray-200"}`}>
                                            ...
                                            <button onClick={() => router.push("/eviko-pos/printers")} className={`w-full text-left px-3 py-2.5 text-sm font-bold rounded-lg transition-colors flex items-center gap-2 ${dark ? "text-gray-200 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}>
                                                <span>🖨️</span> PRINTER SOZLAMALARI
                                            </button>
                                            <button onClick={() => router.push("/eviko-pos/support")} className={`w-full text-left px-3 py-2.5 text-sm font-bold rounded-lg transition-colors flex items-center gap-2 ${dark ? "text-gray-200 hover:bg-gray-700" : "text-gray-700 hover:bg-gray-100"}`}>
                                    ...
                                    <div className="w-full h-[75vh] lg:h-auto lg:flex-1 shrink-0 overflow-hidden lg:border-r border-gray-200">
                                        ...
                                        <MenuPanel
                                            ...
                                            onConfirm={async (cart) => {
                                                ...
                                                // 1. LOCAL STATE ni DARHOL yangilaymiz — foydalanuvchi kutmaslik uchun
                                                setTableOrders(prev => {
                                                    const existing = prev[selTable.id] || [];
                                                    const merged = [...existing];
                                                    modCart.forEach(c => {
                                                        const ex = merged.find((m: any) => m.item.id === c.item.id && !!m.isSaboy === !!c.isSaboy && (m.shotId || 1) === activeShot);
                                                        if (ex) ex.qty += c.qty;
                                                        else merged.push({ ...c });
                                                    });
                                                    return { ...prev, [selTable.id]: merged };
                                                });

                                                // 2. DB ga saqlaymiz (AWAIT qilinmaydi, background da ketadi)
                                                fetch("/api/eviko/orders-db", {
                                                    method: "POST", headers: hdrs,
                                                    body: JSON.stringify({
                                                        tableId: selTable.id,
                                                ...
                                                // Stol statusini darhol yangilash (fon rejimida)
                                                store.fetchSmartTables();
                                            }}
                                                ...
                                                <button
                                                    ...
                                                    onClick={async () => {
                                                        ...
                                                        try {
                                                            ...
                                                            const r = await fetch("/api/eviko/print-kitchen", {
                                                        ...
                                                        <button onClick={() => {
                                                            const doAction = () => {
                                                                ...
                                                                setSelOrder((updatedData: any) => {
                                                                    if (!isTw) {
                                                                        fetch("/api/eviko/yetkazish", { method: "PUT", headers: hdrs, body: JSON.stringify({ id: String(selOrder.id), items: updatedData.items }) }).catch(()=>null);
                                                                    }
                                                                    return updatedData;
                                                                });
                                                            };
                                                    ...
                                                    <div className="flex items-center gap-1.5 flex-shrink-0">
                                                        ...
                                                        <button onClick={async () => {
                                                            ...
                                                            try {
                                                                ...
                                                                const hdrs: Record<string,string> = { "Content-Type": "application/json" };
                                                                if (token) hdrs["Authorization"] = `Bearer ${token}`;
                                                                const r = await fetch("/api/eviko/print", { method: "POST", headers: hdrs,
                                                        ...
                                                        <button onClick={async () => {
                                                            await fetch("/api/eviko/printers", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: p.id }) });
