...
export default function MobileWaiterPage() {
    ...
    const fetchMenu = useCallback(async () => {
        if (!token) return;
        try {
            const res = await fetch("/api/eviko/menu", { headers: { Authorization: `Bearer ${token}` } });
    ...
    const sendOrder = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/eviko/orders-db", {
                method: "POST",
    ...
    const openTable = async (t: SmartTable) => {
        ...
        if (t.status === "occupied" || t.status === "receipt") {
            try {
                const res = await fetch(`/api/eviko/orders-db?tableId=${t.id}`, {
