...
export default function UbtReportsPage() {
    ...
    useEffect(() => {
        ...
        const fetchReturns = async () => {
            try {
                const res = await fetch(buildQuery("/api/eviko/reports-returns?"));
                if (res.ok) {
        ...
        const fetchReportsData = async () => {
            try {
                const res = await fetch(buildQuery("/api/eviko/reports-data?"));
                if (res.ok) {
                    const data = await res.json();
                    setReportsData(data);
                }
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">{t('reports.title') || 'EVIKO Hisobotlari'}</h1>
