...
export default function TaomlarPage() {
    ...
    const loadFromDB = async () => {
        try {
            const res = await fetch("/api/eviko/menu");
            const data = await res.json();
            setDbItems(data.items ?? []);
            setDbCategories(data.categories ?? []);
            
            const ingRes = await fetch("/api/eviko/xomashyo");
                                        ...
                                        <div className="shrink-0">
                                            ...
                                            <div
                                                ...
                                                onDrop={(e) => {
                                                    ...
                                                    // 1. Kompyuterdan lokal fayl tortilganda
                                                    ...
                                                    // 2. Brauzerdan (boshqa tabdan/sahifadan) rasm tortib kelinganda
                                                    const html = e.dataTransfer.getData("text/html");
                                                    if (html) {
                                                        const srcMatch = html.match(/src=["'](.*?)["']/);
                                                        if (srcMatch && srcMatch[1]) {
                                                            setFormData({ ...formData, image: srcMatch[1] });
                                                            return;
                                                        }
                                                    }
