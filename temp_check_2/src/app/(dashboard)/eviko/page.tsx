...
function DebtDetailsModal({ type, onClose, data, loading, onRefresh, onSverkaGenerated }: any) {
    ...
    const handlePay = async (id: string, fullAmount: number, currentPaid: number, title: string) => {
        ...
        setPaying(id);
        try {
            const res = await fetch("/api/eviko/stats/debts", {
                method: "POST",
...
export default function UbtPage() {
    ...
    const dishQuery = dateFilter.range
        ? `/api/eviko/reports-data?from=${dateFilter.range.from}&to=${dateFilter.range.to}`
        : `/api/eviko/reports-data?timeframe=${dateFilter.timeframe}`;
    ...
    const statsUrl = (() => {
        if (period === "custom" && customFrom && customTo)
            return `/api/eviko/stats?period=custom&from=${customFrom}&to=${customTo}`;
        return `/api/eviko/stats?period=${period}`;
    })();
    ...
    useEffect(() => {
        ...
        const ti = setInterval(() => store.fetchSmartTables(), 60_000);
