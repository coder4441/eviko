...
// Har 10 daqiqada eskirgan sessionlarni tozalash
setInterval(() => {
    const now = Date.now();
    _sessionCache.forEach((v, k) => { if (v.expiresAt < now) _sessionCache.delete(k); });
}, 10 * 60_000);
...
export async function authenticateAdmin(shopCode: string | null, username: string, password: string) {
    ...
    try { if ((tenant as any).settings) tenantSettings = typeof (tenant as any).settings === "string" ? JSON.parse((tenant as any).settings) : (tenant as any).settings; } catch {}
 ...
 */
export async function authenticatePlatformUser(phone: string, password: string, agentCode?: string) {
    let normalizedPhone = phone.replace(/\s+/g, "");
    if (!normalizedPhone.startsWith("+")) {
        normalizedPhone = "+" + normalizedPhone; // fallback for users not entering +
    }
    ...
    if (agentCode && user.role === "Agent") {
        if (user.agentCode !== agentCode && user.agentCode !== agentCode.replace("#", "").trim()) {
            return { success: false, error: "Agent kodi noto'g'ri" };
        }
    }
...
export async function authenticateKassir(username: string, password: string, tenantId: string) {
    ...
    try { if (staff.permissions) permissions = typeof staff.permissions === "string" ? JSON.parse(staff.permissions) : (staff.permissions as any); } catch {}
