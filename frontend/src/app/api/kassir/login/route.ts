...
export async function POST(request: NextRequest) {
    try {
        ...
        const staffList = allActiveStaff.filter(s => {
            const storedUsername = s.username.toLowerCase().trim().replace(/^\+/, "");
            const searchClean = searchLower.replace(/^\+/, "");
            if (storedUsername === searchClean) return true;
            if (s.name.toLowerCase().trim() === searchClean) return true;
        ...
        try { tenantSettings = authenticatedTenant.settings || {}; } catch { tenantSettings = {}; }
        const tenantShopType = (tenantSettings.shopType as string) || "shop";

        let parsedPermissions: string[] = [];
        try { parsedPermissions = (authenticatedStaff.permissions as any) || []; } catch { parsedPermissions = []; }

        let staffPrinterIp = "";
        let autoPrintReceipt = true;
        try {
            const phoneData: any = authenticatedStaff.phone || {};
        ...
        if (!staffPrinterIp) {
            try {
                const printers = await prisma.smartPrinter.findMany({
                    where: { tenantId: authenticatedTenant.id },
        ...
        let serviceFeePct = 0;
        try {
            const meta: any = authenticatedStaff.staffMeta || {};
