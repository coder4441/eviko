/**
 * /api/eviko/sync-export — Offline rejim uchun tenant ma'lumotlarini eksport qiladi.
 *
 * VPS da ishlaydi. EXE (agent/kiosk) buni online paytda chaqirib, natijani
 * lokal serverning /api/eviko/sync-import ga yozadi. Shunda offline'ga o'tganda
...
export async function GET(req: NextRequest) {
    try {
        ...
        let categories: any[] = [];
        try {
            categories = await (prisma as any).ubtCategory.findMany({ where: { tenantId } });
