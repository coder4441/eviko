...
export default function PrintersPage() {
    const { t } = useLang();
    const [printers, setPrinters] = useState<SmartPrinter[]>([]);
    ...
    const loadJobs = async () => {
        try {
            const res = await fetch("/api/eviko/print-queue?limit=20", { cache: "no-store" });
            const data = await res.json();
    ...
    const probeAll = async () => {
        setProbing(true);
        try {
            await fetch("/api/eviko/printer-heartbeat", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
            await loadPrinters();
        } catch {}
        setProbing(false);
    };
    ...
    const jobAction = async (id: string, action: "retry" | "cancel") => {
        try {
            await fetch("/api/eviko/print-queue", {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
    ...
    const loadWindowsPrinters = async () => {
        ...
        setError("");
        try {
            const res = await fetch("/api/eviko/agent-printers");
            const data = await res.json();
            setWinPrinters(Array.isArray(data.printers) ? data.printers : []);
            if (!data.printers || data.printers.length === 0) {
                setError("Printer topilmadi. Agar 'agent.js' ishlayotgan bo'lsa biroz kutib yana urinib ko'ring (sinxronlash 30 soniya olishi mumkin).");
            }
        } catch {
            setWinPrinters([]);
    ...
    const loadLanPrinters = async () => {
        ...
        try {
            ...
            const res = await fetch("/api/eviko/discover-printers", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
            const data = await res.json();
            ...
            if (lan.length === 0) {
                try {
                    const res2 = await fetch("/api/eviko/agent-printers");
    ...
    const handlePing = async (p: SmartPrinter) => {
        ...
        try {
            // Xoh USB xoh LAN bo'lsin baribir VPS Backend orqali ishlaydi (ajoyib Polling Arxitekturamiz!).
    ...
    const isUsb = (p: SmartPrinter) => p.ipAddress.startsWith("usb://");
