...
export default function AgentPortalLayout({
    ...
    return (
        <div className="agent-portal-dark min-h-screen flex" style={{ backgroundColor: '#0f172a', color: '#f1f5f9' }}>
            {/* Sidebar */}
            <aside className="w-64 bg-slate-800 border-r border-slate-700 hidden md:flex flex-col" style={{ zIndex: 10, position: 'relative' }}>
                <div className="p-6 border-b border-slate-700">
                    <h2 className="text-xl font-black text-sky-400">Agent Portal</h2>
                    <p className="text-xs text-slate-400">EVIKO CRM</p>
                </div>
            ...
            {/* Main Content — overflow-auto removed so fixed modals are not clipped */}
            <main className="flex-1 min-h-screen md:pt-0 pt-16 overflow-x-hidden">
                {children}
            </main>
        </div>
    );
