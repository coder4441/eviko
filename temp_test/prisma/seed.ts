...
async function main() {
    ...
    for (let i = 0; i < [tenant1, tenant2, tenant3].length; i++) {
        const tenant = [tenant1, tenant2, tenant3][i];
        for (const staff of staffData) {
            const uniqueUsername = `${staff.username}${i + 1}`;
            const passwordHash = await bcryptjs.hash(uniqueUsername + "123", 10);
