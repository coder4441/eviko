...
// ─── GET /api/eviko/orders-db?tableId=xxx ────────────────────────────────────
...
export async function GET(request: NextRequest) {
    try {
        ...
        console.log(`[orders-db GET] Returning ${finalItems.length} items for table ${tableId}`);
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log(`[orders-db POST] Created order ${order.id} for table ${tableId} with ${items.length} items. items:`, JSON.stringify(items));
        ...
        if (!skipAutoPrint) {
            ...
            if (!fallbackPrinterIp) {
                try {
                const fallbackPrinter = await prisma.smartPrinter.findFirst({
                    where: { tenantId: auth.tenantId },
                    orderBy: { createdAt: 'asc' },
                    select: { ipAddress: true }
                });
                if (fallbackPrinter) {
                    fallbackPrinterIp = fallbackPrinter.ipAddress || null;
                    }
            ...
            const noIpItems: any[] = []; // printerIp yo'q taomlar
            ...
            // printerIp yo'q taomlarni fallback ga qo'shish
            ...
            // Agar hech qanday printer guruh bo'lmasa — print qilinmaydi
            if (Object.keys(printerGroups).length === 0) {
                console.log("[orders-db] Printer topilmadi — oshxona cheki chiqarilmadi");
            } else {
                const now = new Date();
                ...
                for (const [printerIp, pItems] of Object.entries(printerGroups)) {
                        ...
                        PrinterServiceModule.PrinterService.print({
                            ...
                            console.log(`[Kitchen Print ✅] ${printerIp} → ${tableNameToPrint}`);
                        }).catch(e => {
                            console.warn(`[Kitchen Print ❌] ${printerIp}:`, e?.message || e);
                        });
                    }
            }
            } catch (printErr) {
...
// ─── DELETE /api/eviko/orders-db?tableId=xxx ────────────────────────────────
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        await prisma.smartTable.updateMany({
            where: { id: tableId, tenantId: auth.tenantId },
