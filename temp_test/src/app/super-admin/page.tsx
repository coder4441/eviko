...
import { 
    Users, Building2, Store, CreditCard, LayoutDashboard, Settings as SettingsIcon, 
    LogOut, Plus, Search, Filter, MoreVertical, Edit2, Trash2, Key, CheckCircle2, 
    XCircle, Clock, ShieldCheck, Download, Smartphone, Check, X, Shield, History, MapPin, Tag, ClipboardList,
    ChevronDown, ChevronUp, Phone, Globe, User, BarChart3, AlertCircle, RefreshCw, Menu, LogIn, DollarSign
} from "lucide-react";
...
import { SettingsManager } from "@/components/super-admin/SettingsManager";
...
type SideTab = "tenants" | "users" | "billing" | "tariffs" | "settings" | "leads";
...
export default function SuperAdminPage() {
    ...
    const canManageSettings = isMaster || currentUser?.permissions?.includes("settings:manage");
    const canViewLeads = isMaster || currentUser?.permissions?.includes("leads:view");
    ...
    const allTariffs = useMemo(() => (tariffsData?.tariffs || []) as any[], [tariffsData?.tariffs]);
    ...
    const handleSave = async () => {
        ...
        if (!editingTenant&&formData.adminPassword.length<4) { alert("Admin paroli kamida 4 ta belgi bo'lishi kerak"); return; }
    ...
    const handleLogout = () => { useSuperAdminStore.getState().logout(); router.push("/"); };
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] text-slate-800 flex overflow-hidden relative">
            ...
            {canViewTenants && (
                ...
                {canViewLeads && (
                    <NavItem icon={ClipboardList} label="Zayavkalar" active={activeTab==="leads"} onClick={()=>setActiveTab("leads")} />
                )}
                ...
                {isMaster && (
                    <>
                        ...
                        <NavItem icon={SettingsIcon} label="Sozlamalar" active={activeTab==="settings"} onClick={()=>{setActiveTab("settings");}} />
                    </>
                )}
                        ...
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-800">
                                {activeTab === "tenants" ? "Tashkilotlar" :
                                 activeTab === "leads" ? "Zayavkalar (Leads)" :
                ...
                <div className="flex-1 overflow-y-auto px-4 sm:px-8 py-4 sm:py-6">
                    ...
                    {activeTab === "tenants" && (
                        <div className="space-y-6">
                            ...
                            {/* Modular Tenants Table (pagination + bulk actions) */}
                            <TenantsManager
                    ...
                    {activeTab === "billing" && (
                        <div className="space-y-6">
                            <BillingDashboard
                                ...
                                onAction={(company: any, type: string) => {
                                    if (type === "subscribe") setBillingSubForm({ tariffId: company.tariffId || "", months: 1 });
                                    if (type === "topup") setBillingTopupForm({ amount: "", note: "" });
                                    setBillingActionModal({ company, type: type as any });
                                }}
                            />
                        </div>
                    )}
                    ...
                    {activeTab === "leads" && (
                        <LeadsTab 
                            onConvertToTenant={(lead) => {
                                setEditingTenant(null);
                                setFormData({
                                    shopCode: `SMS${String(tenants.length + 1).padStart(3, "0")}`,
                                    shopName: lead.businessName,
                                    ...
                                    shopType: "smart",
                                    adminPassword: "",
                                    settings: { ...DEFAULT_SETTINGS, contacts: [] }
                                });
                                setShowExtra(false);
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    {!editingTenant && <div><label className={labelClass}>Admin paroli *</label><input type="password" value={formData.adminPassword} onChange={(e)=>f("adminPassword",e.target.value)} className={inputClass} placeholder="Kamida 4 ta belgi" autoComplete="new-password"/></div>}

                                    <div><label className={labelClass}>Tarif tanlash</label>
                                        <select value={formData.plan} onChange={(e) => {
                                            const selectedTariff = allTariffs.find((t: any) => t.id === e.target.value);
                                            f("plan", e.target.value);
                                            if (selectedTariff?.durationDays) {
                                                fs("subscriptionDays", selectedTariff.durationDays);
                                            }
                                        }} className={inputClass}>
                                            <option value="">— Tarif tanlang —</option>
                                            {allTariffs.filter((t: any) => t.isActive).map((t: any) => (
                                                <option key={t.id} value={t.id}>
                                                    {t.name}{t.pricePerMonth > 0 ? ` — ${Number(t.pricePerMonth).toLocaleString("uz-UZ")} UZS/oy` : " — Bepul"}
                                                </option>
                                            ))}
                                            {allTariffs.filter((t: any) => t.isActive).length === 0 && (
                                                <option disabled>Tariflar topilmadi (Tariflar bo'limidan qo'shing)</option>
                                            )}
                                        </select>
                                    </div>
                                    ...
                                    <div><label className={labelClass}>Tashkilot turi</label>
                                ...
                                {/* Subscription Days - auto from tariff */}
                                <div className="mt-4">
                                    <label className={labelClass}>Obuna muddati <span className="text-sky-400 font-normal">— tarifdan avtomatik</span></label>
                                    <div className="flex items-center gap-3 mt-2">
                                        <div className={`${inputClass} w-36 text-center text-2xl font-black text-sky-300 bg-slate-50 cursor-not-allowed select-none flex items-center justify-center`}>
                                            {formData.settings.subscriptionDays || 0}
                                        </div>
                                        <div>
                                            <p className="text-sm font-semibold text-slate-700">kun obuna</p>
                                            <p className="text-xs text-slate-400 mt-0.5">
                                                {formData.plan
                                                    ? `Tarif tanlanganda avtomatik o'rnatildi`
                                                    : "Avval tarif tanlang"}
                                                {(formData.settings.subscriptionDays || 0) > 0 && (
                                                    <> — akk <span className="text-red-400 font-bold">{formData.settings.subscriptionDays} kundan so'ng bloklashga tushadi</span></>
                                                )}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <section>
                                <h3 className="text-sm font-semibold text-sky-400 uppercase tracking-wider mb-4 flex items-center gap-2"><SettingsIcon size={16}/>Sozlamalar</h3>
                            ...
                            <section>
                                <button type="button" onClick={() => setShowExtra(v => !v)}
                                    ...
                                    <span className="flex items-center gap-2">
                                        <SettingsIcon size={15}/> Qo&apos;shimcha sozlamalar
                                    </span>
