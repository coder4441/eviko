...
const getStorageKey = (): string => {
    if (typeof window !== "undefined") {
        ...
        return activeShop ? `eviko-storage-v2-${activeShop}` : "eviko-storage-v2";
    }
    return "eviko-storage-v2";
};
        ...
        (set) => ({
            ...
            fetchSmartTables: async () => {
                try {
                    ...
                    const res = await fetch(`/api/eviko/tables?_t=${Date.now()}`, { headers, cache: "no-store" });
                    if (res.status === 401) {
            ...
            fetchKdsOrders: async () => {
                try {
                    const res = await fetch(`/api/eviko/orders?_t=${Date.now()}`, { cache: "no-store" });
                    if (res.ok) {
                        const data = await res.json();
                        if (data.orders) {
            ...
            addKDSOrder: async (tableId, description) => {
                try {
                    const res = await fetch("/api/eviko/orders", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
            ...
            payTable: async ({ tableId, items, paymentMethod, total, waiterName, tableLabel }) => {
                try {
                    const res = await fetch("/api/eviko/pay", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
